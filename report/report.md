...
|---|---|---|---|---|---|---|
| **1. Instrumentation downloads** — npm `dd-trace`, `datadog-metrics`; controls `newrelic`, `elastic-apm-node`; placebo `lodash`, `chalk`, `axios`, `react`; PyPI `ddtrace` as a cross-ecosystem check | Daily downloads, npm registry API and pypistats | Datadog bills on hosts, metrics, spans and logs; customers emit telemetry by installing instrumentation, so downloads proxy the billable unit. Contemporaneous corr with `rev_yoy` 0.79 (n=27) | Daily · 1-day latency · npm 2015+, **PyPI only 181 days** · **Node.js APM only, ~10% of observable install volume** | Free (PyPI history needs BigQuery, out of scope) | CI re-pulls ≠ installs; mirrors; no revenue weighting; **one SDK of a multi-product platform** → YoY log differences, control and placebo baskets, constant composition, outage imputation, count-type targets. PyPI window too short to confirm or refute | **Tested** |
...
| **3. Earnings-release text** — management-authored body of the 8-K Exhibit 99.1; placebo is counsel's forward-looking-statements boilerplate *in the same document* | Item 2.02 8-K press releases, cached for all 28 quarters | Guidance conservatism may signal a lower bar the company intends to clear (sandbagging). Timestamped **at the moment guidance is issued**, so the as-of vintage is exact by construction | Quarterly · 0-day latency · 28 quarters · every reported quarter | Free | Lexical drift tracks time, not information → in-document placebo, which is what exposed the failure (§3.6) | **Tested** |
| **4. Hiring / job postings** — postings requiring "Datadog"; sales headcount | Revelio, LinkUp, Coresignal | Adoption breadth plausibly leads bookings 2–3 quarters; headcount is future capacity | Daily/weekly · days · vendor-dependent · US-skewed | **Vendor, paid** | No free source with usable history; over-weights large employers → de-seasonalise, employer fixed effects | **Proposed** |
| **5. Competitive displacement / cost sentiment** — Stack Exchange tags; GitHub vs `grafana/grafana`, OpenTelemetry | Public APIs | **Downside-risk proxy for NRR, not a revenue predictor.** Bill shock is a documented churn vector | Daily · days · 2010+ | Free | Volume tracks community size, not spend → share-of-tag construction, matched competitor baseline. **Not forced into the revenue model** | **Proposed** |
...
Features are stored as **vintages** — (quarter, feature, value, `available_from`) — so requesting a feature set is a filter, not a judgement. 1,666 rows, 50 features. `available_from` is the earnings 8-K date, not the 10-Q filing date (the 8-K leads by a median of 1 day, up to 18 for Q4). Reporting lag measured, not assumed: median 38 days, range 34–47. Thirteen unit tests enforce this, including a static check that the builder never references the look-ahead-contaminated imputation variant, and a dynamic one that replaces that loader with a raising stub and rebuilds the panel.

Two cleaning rules were fixed before their effect on any result was known. **Constant basket composition** — only packages present at sample start, since adding the three launched later injects a permanent 0.31 log-point jump. And the 12 registry-wide outage days, four inside the live quarter, are filled **backward-only** from the same weekday in the prior 42 days; a centred fill uses days *after* the gap and is look-ahead by construction.
...
An earlier version used a trailing 8-quarter window chosen after seeing the sample — data snooping on the baseline while the signals faced walk-forward discipline. Replaced: **`guidance + auto-window beat`** selects the window from {4, 6, 8, 12, expanding} by nested walk-forward *inside the training set only*.
...
**Removing the snooping made the baseline stronger** (RMSE 0.0229 → 0.0200; MAPE 5.48% → 4.10%), raising the bar for the signals. For the live quarter the fair rule independently selects 8 quarters, so the published call is unchanged — same number, legitimate derivation.
...
**The chain that explains it.** Against AR(1), 15 of 24 cells score below 0.9 while a permutation null (1,000 draws) yields 0.58 — **p = 0.002**, so the features carry information AR(1) lacks. But a **bare time index** through the identical pipeline also beats AR(1) (0.896) with zero Datadog content, and against the strongest baseline that null yields 0.01 cells against an observed **0**. The information was drift, which a correctly specified naive model already supplies.
...
The Objective names billings/RPO, NRR and $100k+ ARR customer growth. These are prediction **targets (Y)**, not alternative data sources (X), so testing them widens the target set rather than the signal set. It also tests a hypothesis the download signal implies: **downloads are an unweighted count while revenue is dollar-weighted**, so a count-type target should match better.
...
**Directionally supported, still insufficient.** Against customer growth the best cell is **1.049** — the closest any signal came to a baseline here, versus 1.137 against revenue growth. Matched controls fail on those cells (1.69–1.83), so the residual edge is attributable. Still a loss, at n=11.

### 3.6 One unstructured signal, and the sharpest placebo result in the project

Management tone is the one qualitative signal that is cleanly backfillable — free, cached for 28 quarters, timestamped *at the guidance-issuance moment*. Hypothesis: heavier hedging means a lower bar, so a larger beat. The design point is the placebo: every release carries a forward-looking-statements disclaimer written by counsel, not management, and tone measured there should predict nothing.
...
**The legal disclaimer outperforms management's own words on every comparison**, and against AR(1) yields 0.627, CI [0.533, 0.835], DM p=0.088 — a "significant" result from text written to convey no information. Boilerplate drifts as counsel updates the template, so it proxies time. The implication inverts the intuition: text features are **more** exposed to spurious trend-fitting than structured ones, so control discipline matters more than extraction quality. *(Compact lexicon, not full Loughran-McDonald; a better one moves the estimates, not the comparison.)*

---

## 4. Limitations and what the data does not support

**Power, quantified rather than conceded.** At n=13 a Diebold–Mariano test detects an RMSE ratio of 0.95 only **6%** of the time, 0.90 **15%**, 0.80 **28%** — **a genuine 5–10% edge would have gone undetected 85–90% of the time**. So "0 of 24" *bounds* the effect size rather than showing it is zero. Not a power problem: the observed cells sit at **1.05 to 2.65**, consistently on the wrong side of parity.

**The interval is conditional.** ±$14.7m reflects only the historical variance of the beat. Guidance extraction error is zero — every figure re-fetched from EDGAR and matched to its verbatim outlook sentence. Regime risk is second-order: flat trailing-mean gives $1,188m against $1,195m trend-extrapolated, a spread inside the band. Not covered: guidance-philosophy change, customer concentration, M&A (Datadog acquired Adaptive ML in the quarter). Formally the band is *conditional on the beat distribution remaining stationary* — supported over 16 quarters, mildly strained over 8 (ρ=+0.69, p=0.058).

**Sample size.** 27 usable quarters, 7–14 out-of-sample points by target. Every hit rate here is indistinguishable from chance (binomial p ≥ 0.18); Granger tests are descriptive only; models are OLS with at most two predictors. Relatedly, a result that flips on 0.34% of the data is not a result: removing 12 outage days out of 3,512 moves the cross-registry correlation from below its placebo (0.932 vs 0.951) to above it (0.972 vs 0.961) — grounds for the inconclusive verdict, not a repair.

---

## 5. Synthesis: the binding constraint is observability, not modelling

Three findings say the same thing.

**The free data covers the wrong tenth of the business.** The Go agent that collects the billable telemetry ships through Docker Hub, APT/YUM, Helm and marketplaces — **11.25bn cumulative pulls on its main channel against 1.13bn on the npm basket measured here** — and roughly 90% of that surface publishes no time series. No modelling recovers a series that was never recorded.

**Within that tenth, the proxy has decayed.**
...
Cross-sell and tiering are real — revenue per large customer rose 67% — but downloads per large customer rose **roughly ten times faster**, so per-customer monetisation explains a minority of the gap. Private-registry mirroring pushes the *opposite* way. **CI/CD re-pulls fail too:** the weekday/weekend ratio went 6.39 → 5.61, slope p=0.77. Remaining candidates — mirror traffic, container rebuilds, AI coding agents — are named, not implied to be ruled out.

A perfect model of npm downloads would still be modelling a quantity whose link to revenue is not stable.

**What looked like signal was drift, and the controls showed it.** Three signals, 24 cells, four targets, none beating a correctly specified naive baseline. A bare time index beat AR(1); a legal disclaimer "predicted" surprises at r=−0.81. Both were caught by controls, not by models.

**The conclusion is therefore scoped, not global:** *the freely observable slice is unrepresentative and its coupling to revenue is decaying, and within it the apparent predictive power is not distinguishable from common trend.*

### What I would do next, in order

1. **Start the Docker Hub daily snapshot today.** One API call a day turns a lifetime counter into a delta series covering the 90% of the install base currently invisible. It cannot be backfilled, so every day of delay loses a day of history permanently — highest value, near-zero cost.
2. **Price the dollar-weighted alternative.** The unfixable flaw in download counts is that one enterprise and one hobbyist count the same; card and invoice panels are dollar-weighted by construction. Run the observability triage on a vendor's coverage before buying.
3. **Re-run the pipeline quarterly and let it change its mind.** Three conditions reverse the conclusion: the downloads-to-revenue ratio stabilises; a candidate beats the *strongest* baseline while its placebo fails; the result replicates in a second ecosystem. All three are automated tests, not judgement calls.
4. **Until then the guidance-anchored rule is the working model.** MAPE 4.10%, hit rate 76.9%, no alternative data — and the benchmark any future signal must beat.
