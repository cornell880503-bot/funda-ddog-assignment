...
app.appendChild(el(panel("Headline nowcast &mdash; Q3 2026 revenue", `
  ...
  ${(() => {
    const W = 760, HH = 92, PADL = 14, PADR = 14;
    const lo = Math.min(H.guide_low, H.lo) - 18, hi = Math.max(H.guide_high, H.hi) + 18;
    const x = v => PADL + (v - lo) / (hi - lo) * (W - PADL - PADR);
    return `<svg viewBox="0 0 ${W} ${HH}" width="100%" height="${HH}" style="margin:12px 0 4px"
      role="img" aria-label="Forecast interval against the guidance range">
      <rect x="${x(H.guide_low)}" y="24" width="${Math.max(2, x(H.guide_high) - x(H.guide_low))}" height="18"
        rx="3" fill="var(--dim)" opacity="0.6"/>
      <text x="${x(H.guide_mid)}" y="18" text-anchor="middle" font-size="10"
        fill="var(--muted)" font-family="var(--mono)">guidance ${fmt(H.guide_low,0)}&ndash;${fmt(H.guide_high,0)}</text>
      <rect x="${x(H.lo)}" y="48" width="${x(H.hi) - x(H.lo)}" height="18"
        rx="3" fill="var(--accent)" opacity="0.32"/>
      <line x1="${x(H.lo)}" y1="46" x2="${x(H.lo)}" y2="68" stroke="var(--accent)" stroke-width="1.5"/>
      <line x1="${x(H.hi)}" y1="46" x2="${x(H.hi)}" y2="68" stroke="var(--accent)" stroke-width="1.5"/>
      <text x="${x(H.lo)}" y="42" text-anchor="middle" font-size="9" fill="var(--accent)"
        font-family="var(--mono)">${fmt(H.lo,0)}</text>
      <text x="${x(H.hi)}" y="42" text-anchor="middle" font-size="9" fill="var(--accent)"
        font-family="var(--mono)">${fmt(H.hi,0)}</text>
      <line x1="${x(H.point)}" y1="42" x2="${x(H.point)}" y2="72" stroke="var(--text)" stroke-width="2.5"/>
      <circle cx="${x(H.point)}" cy="57" r="4.5" fill="var(--text)"/>
      <text x="${x(H.point)}" y="86" text-anchor="middle" font-size="11.5" font-weight="700"
        fill="var(--text)" font-family="var(--mono)">$${fmt(H.point,1)}m</text>
    </svg>
    <div class="note" style="margin-top:0">Grey: company guidance. Blue: the 95%
